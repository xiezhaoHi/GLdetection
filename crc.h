...
unsigned int   N_CRC16(unsigned char *updata, unsigned int len)
{
	...
	unsigned int lenData = len - 2;
	unsigned char* tempData = updata;
	while (lenData--)
	{
	...
	unsigned int ret = ((uchCRCHi << 8) | uchCRCLo)&(~0x8000); //(uchCRCHi << 8 | uchCRCLo);
	unsigned char t1 = *updata++;
	unsigned char t2 = *updata;
	unsigned int res = ((t1 << 8) | t2)&(~0x8000);
	return ret == res ? 0 : -1;
}
