...
#define  BackGroundRGB  RGB(54,63,69)
	LinearGradientBrush lgb(Point(0, 0), Point(destRect.Width, destRect.Height), BackGroundRGB, BackGroundRGB);
	...
	if (m_pWnd)
	{
			HICON hIcon = m_pWnd->GetIcon(false);
			if (hIcon)
			{
...
BOOL  CSkinWndHelper::Attach(HWND hWnd, CWnd* pwnd)
{
	m_pWnd = pwnd;
	m_hWnd = hWnd;
	...
	rcControl.top = 20;
	rcControl.right = rcControl.left + 24;
	rcControl.bottom = rcControl.top + 24;
...
// 	}
	...
	if (m_bHaveMaxBox||m_bHaveMinBox)
	{
		rcControl.left = rcControl.left - 24-20;
		rcControl.right = rcControl.left + 24;
